...
| **B2** | Readwrite splitting | Insert + select through `write_ds` / `read_ds_*`; empty/dup PK; concurrent write/read; same-host different-DATABASE routing; T3=A/Q-05a RO deny (INSERT/UPDATE/DELETE/DROP) + cleanup when lab allows | Physical replica isolation or lag; multi-machine topology |
